...
def analyzeLayout(PHI, THETA, tol):
    ...
    for idx, phi in enumerate(PHI):
        ...
        for jdx in range(idx + 1, mp):
            ...
            if (angular_distance(phi, THETA[idx], -phj, THETA[jdx]) < tol):
                ...
                if (phi > 0 and phj < 0):
                    paired[idx] = tdx; paired[jdx] = -tdx;
                elif (phi < 0 and phj > 0):
                    paired[idx] = -tdx; paired[jdx] = tdx;
                else:
                    sys.exit(
                        "Well... there is a problem in analyzeLayout function: \nthe pair is not left/right symmetric. phi = %.2f phj = %.2f" % (
                        phi, phj))
    ...
    if (len(paired[abs(paired) >= 1]) % 2 != 0): sys.exit(
        "Something went wrong when matching pairs in analyzeLayout. The number of matched pairs is: %d" % (
        len(paired[paired >= 1])))
